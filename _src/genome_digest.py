...
def do_genome_digest(file_genome, file_annotation):
    ...
    with open(os.path.join(DATA_GENOME, "digest_from_" + file_genome[:max(file_genome.find(".fa"), file_genome.find(".fna"))].replace(DATA_GENOME, '').replace('\\', '').replace('/', '') + ".md")) as file:
        file.write(f"""
